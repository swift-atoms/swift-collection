...
|---------|---------------|--------------|
| `Collection` | Collection-owned composition | The collection protocol family, views, operations, and standard-library bridges. Import focused Sequence terminal products such as `Sequence ForEach` explicitly when those operations are needed. |
...
`Collection.Protocol` refines `Sequence.Borrowing.Protocol`, so every conformer vends a span-based `makeIterator()`. Terminal operations remain opt-in through focused products such as `Sequence ForEach`, `Sequence Reduce`, `Sequence Contains`, and `Sequence First`. It does **not** refine `Sequenceable` (the single-pass / consuming attachable) — that is an orthogonal capability. Bridging to `Swift.Collection` or `for-in` additionally needs a `Swift.Sequence`-compatible `makeIterator()`, since the borrowing witness is a chunk iterator rather than a scalar `Swift.IteratorProtocol`.
